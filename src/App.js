...
function App() {
  ...
  const [imageURL, setImageURL] = useState(null);
	...
	return (
		<div className="App">
			<h1 className='header'>Naitilus Web App</h1>
						...
						{results.map((result, index) => {
							return (
								<div className='result' key={result.className}>
									...
									<span className='confidence'>Nivel de confiabilidad: {(result.probability * 100).toFixed(2)}% {index === 0 && <span className='bestGuess'>Best Guess</span>}</span>
								</div>
							)
						})}
					</div>}
